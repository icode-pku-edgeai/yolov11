...
class LoadStreams:
    ...
    def update(self, i, cap, stream):
        """Read stream frames in daemon thread and update image buffer."""#在守护线程中读取流帧并更新图像缓冲区
    ...
    def close(self):
        """Terminates stream loader, stops threads, and releases video capture resources."""#终止流加载器，停止线程，并释放视频捕获资源
        self.running = False  # stop flag for Thread停止线程
        for thread in self.threads:
            if thread.is_alive():
                thread.join(timeout=5)  # Add timeout
        for cap in self.caps:  # Iterate through the stored VideoCapture objects
            try:
                cap.release()  # release video capture释放视频捕获资源
...
class LoadScreenshots:
    ...
    Ultralytics 屏幕截图数据加载器，用于捕获和处理屏幕图像
...
class LoadImagesAndVideos:
    ...
    def _new_video(self, path):
        ...
        #为给定路径创建一个新的视频捕获对象，并初始化与视频相关的属性
        self.frame = 0
...
def autocast_list(source):
    ...
    #将一系列来源合并为Numpy数组列表或PIL图像列表，以供Ultralytics进行预测
    files = []
    for im in source:
