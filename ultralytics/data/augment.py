...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz=640, p=1.0, n=4):
        ...
        assert n in {4, 9}, "grid must be equal to 4 or 9."#支持2*2或者3*3
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            # Load image
            img = labels_patch["img"]
            h, w = labels_patch.pop("resized_shape")#此处为随机宽高，不是原图宽高
            ...
            img4[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax] a是拼接后图片位置,b为拼接前位置
            padw = x1a - x1b
            padh = y1a - y1b

            labels_patch = self._update_labels(labels_patch, padw, padh)#更新标签
            mosaic_labels.append(labels_patch)
        final_labels = self._cat_labels(mosaic_labels)
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        """
        r = np.random.beta(32.0, 32.0)  # mixup ratio, alpha=beta=32.0 随机透明度叠加
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if self.auto:  # minimum rectangle 保证为stride的整数倍
            dw, dh = np.mod(dw, self.stride), np.mod(dh, self.stride)  # wh padding
        elif self.scaleFill:  # stretch 直接拉伸无需填充
            dw, dh = 0.0, 0.0
        ...
        if shape[::-1] != new_unpad:  # resize调整尺寸
...
class CopyPaste(BaseMixTransform):
    ...
    def __call__(self, labels):
        ...
        # Get index of one or three other images 随机调用多图
        indexes = self.get_indexes()
        if isinstance(indexes, int):
            indexes = [indexes]
    ...
    def _transform(self, labels1, labels2={}):
        """Applies Copy-Paste augmentation to combine objects from another image into the current image."""#调用多图叠加
        im = labels1["img"]
        cls = labels1["cls"]
        h, w = im.shape[:2]
...
class Format:
    ...
    def _format_img(self, img):
        ...
        将Numpy数组格式的图像转换为PyTorch张量以适配YOLO
