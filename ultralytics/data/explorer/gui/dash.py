...
def _get_explorer():
    """Initializes and returns an instance of the Explorer class."""#初始化并返回一个Explorer类的实例
...
def init_explorer_form(data=None, model=None):
    """Initializes an Explorer instance and creates embeddings table with progress tracking."""#初始化一个Explorer类的实例,创建一个带有进度跟踪功能的嵌入表
    if data is None:
...
def query_form():
    """Sets up a form in Streamlit to initialize Explorer with dataset and model selection."""#设置一个表单，用于初始化Explorer对象，并通过该表单选择数据集和模型
...
def ai_query_form():
    """Sets up a Streamlit form for user input to initialize Explorer with dataset and model selection."""##设置一个表单，用于初始化Explorer对象，并通过该表单选择数据集和模型
...
def find_similar_imgs(imgs):
    """Initializes a Streamlit form for AI-based image querying with custom input."""#在Streamlit框架中初始化一个表单，该表单用于基于人工智能（AI）的图像查询，并支持自定义输入
...
def similarity_form(selected_imgs):
    """Initializes a form for AI-based image querying with custom input in Streamlit."""#在Streamlit框架中初始化一个表单，该表单用于基于人工智能（AI）的图像查询，并支持自定义输入
...
def run_sql_query():
    """Executes an SQL query and returns the results."""#执行sql查询
...
def run_ai_query():
    """Execute SQL query and update session state with query results."""#执行SQL查询并使用查询结果来更新会话状态
...
def reset_explorer():
    """Resets the explorer to its initial state by clearing session variables."""#重置
