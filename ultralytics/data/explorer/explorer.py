...
class ExplorerDataset(YOLODataset):
    ...
    #对YOLODataset进行扩展，以便在模型训练工作流中实现高级数据探索和操作。
    ...
    def load_image(self, i: int) -> Union[Tuple[np.ndarray, Tuple[int, int], Tuple[int, int]], Tuple[None, None, None]]:
        """Loads 1 image from dataset index 'i' without any resize ops."""#从数据集的索引 'i' 位置加载一张图片，并且在这个过程中不对图片进行任何尺寸调整操作
    ...
    def build_transforms(self, hyp: IterableSimpleNamespace = None):
        """Creates transforms for dataset images without resizing."""#转换图像，不包括尺寸调整
        return Format(
            bbox_format="xyxy",
            normalize=False,
...
class Explorer:
    ...
    def create_embeddings_table(self, force: bool = False, split: str = "train") -> None:
        ...
        #创建一个LanceDB数据库表,用于存储数据集中图像的嵌入表示
        Args:
    ...
    def _yield_batches(self, dataset: ExplorerDataset, data_info: dict, model: YOLO, exclude_keys: List[str]):
        """Generates batches of data for embedding, excluding specified keys."""#为嵌入任务生成数据批次，并且在生成过程中会排除指定的键
    ...
    def sql_query(
        ...
        #在数据库表上执行类似SQL查询的操作,并且这个操作利用了LanceDB的谓词下推优化技术
        Args:
    ...
    def plot_sql_query(self, query: str, labels: bool = True) -> Image.Image:
        """
        Plot the results of a SQL-Like query on the table.#SQL查询绘图
