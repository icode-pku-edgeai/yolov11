...
class STrack(BaseTrack):
    ...
    @staticmethod
    def multi_predict(stracks):#执行多目标预测
    ...
    def activate(self, kalman_filter, frame_id):
        ...
        #使用提供的卡尔曼滤波器激活一个新的轨迹段，并初始化其状态和协方差
    ...
    def convert_coords(self, tlwh):#tlwh2xyah
        """Convert a bounding box's top-left-width-height format to its x-y-aspect-height equivalent."""
        return self.tlwh_to_xyah(tlwh)

    @property
    def tlwh(self):#返回tlwh
    ...
    @staticmethod
    def tlwh_to_xyah(tlwh):#tlwh2xyah
...
class BYTETracker:
    ...
    def __init__(self, args, frame_rate=30):
        ...
        Examples:
            ...
            >>> tracker = BYTETracker(args, frame_rate=30)
        """
        self.tracked_stracks = []  # type: list[STrack] #跟踪列表
        self.lost_stracks = []  # type: list[STrack]    #丢失列表
        self.removed_stracks = []  # type: list[STrack]#移除列表
    ...
    def update(self, results, img=None):#新的检测更新跟踪器并返回
        ...
        remain_inds = scores >= self.args.track_high_thresh#划分高低分数
        ...
        # Add newly detected tracklets to tracked_stracks添加新检测到的轨迹片段
        unconfirmed = []
        ...
        # Step 2: First association, with high score detection boxes"首次关联，使用高得分的检测框。"
        strack_pool = self.joint_stracks(tracked_stracks, self.lost_stracks)
        # Predict the current location with KF"使用卡尔曼滤波器预测当前的位置。"
        ...
        matches, u_track, u_detection = matching.linear_assignment(dists, thresh=self.args.match_thresh)#线性匹配
        ...
        # Step 4: Init new stracks初始化新的跟踪
    ...
    def multi_predict(self, tracks):#调用多卡尔曼预测
    ...
    @staticmethod
    def joint_stracks(tlista, tlistb):#"将两个STrack对象列表合并为一个列表，确保根据轨迹ID不重复。"
    ...
    @staticmethod
    def sub_stracks(tlista, tlistb):#"从第一个列表中筛选出存在于第二个列表中的stracks。"
        """Filters out the stracks present in the second list from the first list."""
        track_ids_b = {t.track_id for t in tlistb}
        return [t for t in tlista if t.track_id not in track_ids_b]

    @staticmethod
    def remove_duplicate_stracks(stracksa, stracksb):#"根据交并比（IoU）距离从两个列表中移除重复的stracks。"
