...
class KalmanFilterXYAH:
    ...
    #在图像空间中使用卡尔曼滤波器跟踪边界框,和deepsort里面的大同小异
    通用格式(x, y, a, h, vx, vy, va, vh)
    ...
    def __init__(self):
        ...
        # Create Kalman filter model matrices 创建卡尔曼滤波器的模型矩阵
        ...
        # the amount of uncertainty in the model.位置和速度权重系数
    ...
    def initiate(self, measurement: np.ndarray) -> tuple:
        ...
        return mean, covariance#用测量值初始化均值和方差
    ...
    def predict(self, mean: np.ndarray, covariance: np.ndarray) -> tuple:
        ...
        return mean, covariance#使用运动矩阵将前帧均值、方差，生成噪音项，用来预测后帧的均值和方差
    ...
    def project(self, mean: np.ndarray, covariance: np.ndarray) -> tuple:
        ...
        return mean, covariance + innovation_cov#使用更新矩阵将前帧均值、方差，生成噪音项，用来投影后帧的均值和方差
    ...
    def update(self, mean: np.ndarray, covariance: np.ndarray, measurement: np.ndarray) -> tuple:
        ...
        ).T#使用乔莱斯基分解求卡尔曼增益：投射方差*卡尔曼增益=前帧方差*更新矩阵
        innovation = measurement - projected_mean

        new_mean = mean + np.dot(innovation, kalman_gain.T)
        new_covariance = covariance - np.linalg.multi_dot((kalman_gain, projected_cov, kalman_gain.T))
        return new_mean, new_covariance#用卡尔曼增益更新均值和方差
    ...
    def gating_distance(
        ...
        if metric == "gaussian":
            ...
            cholesky_factor = np.linalg.cholesky(covariance)#乔莱斯基分解
            z = scipy.linalg.solve_triangular(cholesky_factor, d.T, lower=True, check_finite=False, overwrite_b=True)#计算方差到偏差的系数矩阵
            return np.sum(z * z, axis=0)  # square maha得到平方马氏距离
        else:
...
class KalmanFilterXYWH(KalmanFilterXYAH):
    ...
    (x, y, w, h, vx, vy, vw, vh)格式的卡尔曼滤波器，bytetrack的改进版本
    ...
    def predict(self, mean, covariance) -> tuple:
        ...
        return mean, covariance#运动矩阵预测后帧
    ...
    def project(self, mean, covariance) -> tuple:
        ...
        mean = np.dot(self._update_mat, mean)
        covariance = np.linalg.multi_dot((self._update_mat, covariance, self._update_mat.T))
        return mean, covariance + innovation_cov#更新矩阵投射后帧
    ...
    def update(self, mean, covariance, measurement) -> tuple:
        ...
        Examples:
            ...
            >>> new_mean, new_covariance = kf.update(mean, covariance, measurement)
        """
        return super().update(mean, covariance, measurement)#更新方式相同
