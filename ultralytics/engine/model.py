...
class Model(nn.Module):
    ...
    一个用于实现YOLO模型的基类,用于统一不同模型类型的API
    ...
    def train(
        ...
        **kwargs,
        ...
        overrides = yaml_load(checks.check_yaml(kwargs["cfg"])) if kwargs.get("cfg") else self.overrides#加载config配置
        custom = {
            # NOTE: handle the case when 'cfg' includes 'data'. 加载数据、模型、任务
        ...
        if args.get("resume"):
            args["resume"] = self.ckpt_path#加载断点存续

        self.trainer = (trainer or self._smart_load("trainer"))(overrides=args, _callbacks=self.callbacks)
        if not args.get("resume"):  # manually set model only if not resuming 手动设置模型
            self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml)
            self.model = self.trainer.model

        self.trainer.hub_session = self.session  # attach optional HUB session
        self.trainer.train() #执行训练
        # Update model and cfg after training
        if RANK in {-1, 0}:
            ckpt = self.trainer.best if self.trainer.best.exists() else self.trainer.last
            self.model, _ = attempt_load_one_weight(ckpt)
            self.overrides = self.model.args
            self.metrics = getattr(self.trainer.validator, "metrics", None)  # TODO: no metrics returned by DDP 计算指标
        return self.metrics
