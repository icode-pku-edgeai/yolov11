...
def export_formats():
    """Ultralytics YOLO export formats."""#可选的输出格式
    x = [
...
def gd_outputs(gd):
    """TensorFlow GraphDef model output node names."""#TensorFlow GraphDef输出节点名称
...
def try_export(inner_func):
    """YOLOv8 export decorator, i.e. @try_export."""#YOLOv8 导出装饰器
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        """Returns list of exported files/dirs after running callbacks."""#在运行回调后返回导出的文件/目录列表
        ...
        if fmt not in fmts:
            ...
            # Get the closest match if format is invalid 获取最接近的匹配项
        ...
        # Checks 检测名称、half、尺寸、优化工具、系统、world模型、int8校准集、
        ...
        y = None
        for _ in range(2):
            y = model(im)  # dry runs 预热两次
        if self.args.half and onnx and self.device.type != "cpu":
            im, model = im.half(), model.half()  # to FP16 gpu和onnx下转fp16
    ...
    def get_int8_calibration_dataloader(self, prefix=""):
        """Build and return a dataloader suitable for calibration of INT8 models."""#构建并返回一个适用于INT8模型校准的数据加载器
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        """YOLOv8 ONNX export."""#onnx导出
        requirements = ["onnx>=1.12.0"]
        if self.args.simplify:
            requirements += ["onnxslim==0.1.34", "onnxruntime" + ("-gpu" if torch.cuda.is_available() else "")]
        check_requirements(requirements)#检查onnx、onnx-simplify、onnxruntime版本
        import onnx  # noqa

        opset_version = self.args.opset or get_latest_opset()#opset版本
        LOGGER.info(f"\n{prefix} starting export with onnx {onnx.__version__} opset {opset_version}...")
        f = str(self.file.with_suffix(".onnx"))

        output_names = ["output0", "output1"] if isinstance(self.model, SegmentationModel) else ["output0"]#输出名设定
        dynamic = self.args.dynamic#动态导出设定，bool
        if dynamic:
            ...
            if isinstance(self.model, SegmentationModel):
                ...
                dynamic["output0"] = {0: "batch", 2: "anchors"}  # shape(1, 84, 8400) 检测导出设定
        ...
        # Checks
        model_onnx = onnx.load(f)  # load onnx model 加载检查
        ...
        if self.args.simplify:
            try:
                ...
                model_onnx = onnxslim.slim(model_onnx) #调onnx-simplify
    ...
    def export_engine(self, prefix=colorstr("TensorRT:")):
        """YOLOv8 TensorRT export https://developer.nvidia.com/tensorrt."""#trt的导出
        ...
        try:
            ...
            if LINUX:
                check_requirements("tensorrt>7.0.0,<=10.1.0")#版本检查
            import tensorrt as trt  # noqa
        check_version(trt.__version__, ">=7.0.0", hard=True)
        check_version(trt.__version__, "<=10.1.0", msg="https://github.com/ultralytics/ultralytics/pull/14239")

        # Setup and checks 启动trt并检查
        ...
        # Engine builder 设置trt的构建器、配置、工作空间、网络、显式batch、网络、半精度、int8、剖析器
        ...
        if self.args.dynamic:#动态设置
            shape = self.im.shape
            if shape[0] <= 1:
                LOGGER.warning(f"{prefix} WARNING ⚠️ 'dynamic=True' model requires max batch size, i.e. 'batch=16'")
            profile = builder.create_optimization_profile()#优化配置文件
            min_shape = (1, shape[1], 32, 32)  # minimum input shape
            max_shape = (*shape[:2], *(max(1, self.args.workspace) * d for d in shape[2:]))  # max input shape 设定最小最大shape
            for inp in inputs:
                profile.set_shape(inp.name, min=min_shape, opt=shape, max=max_shape)
            config.add_optimization_profile(profile)#保存到配置文件中

        LOGGER.info(f"{prefix} building {'INT8' if int8 else 'FP' + ('16' if half else '32')} engine as {f}")
        if int8:#int8配资
            config.set_flag(trt.BuilderFlag.INT8)#配置int8标志位
            config.set_calibration_profile(profile)#设置校准优化器
            config.profiling_verbosity = trt.ProfilingVerbosity.DETAILED#设置详细性能分析
            ...
            class EngineCalibrator(trt.IInt8Calibrator):
                def __init__(
                    ...
                    self.algo = trt.CalibrationAlgoType.ENTROPY_CALIBRATION_2#设置量化校准算法
                    self.batch = batch
                ...
                def get_batch(self, names) -> list:
                    ...
                    try:
                        im0s = next(self.data_iter)["img"] / 255.0#前处理
                ...
                def read_calibration_cache(self) -> bytes:#读写校准集cache
            ...
            # Load dataset w/ builder (for batching) and calibrate 配置int8校准器
            ...
            config.set_flag(trt.BuilderFlag.FP16)#配置fp16半精度
        ...
        with build(network, config) as engine, open(f, "wb") as t:#模型构建
            # Metadata
