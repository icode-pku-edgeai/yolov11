...
class Results(SimpleClass):
    ...
    def _apply(self, fn, *args, **kwargs):
        ...
        对所有非空属性应用一个函数，并返回一个具有修改后属性的新 Results 对象
    ...
    def plot(
        ...
        if img is None and isinstance(self.orig_img, torch.Tensor):
            img = (self.orig_img[0].detach().permute(1, 2, 0).contiguous() * 255).to(torch.uint8).cpu().numpy()#预测的图转BGR、反归一化、转cpu、转numpy
        ...
        # Plot Classify results输出分类结果
        if pred_probs is not None and show_probs:
            text = ",\n".join(f"{names[j] if names else j} {pred_probs.data[j]:.2f}" for j in pred_probs.top5)
            x = round(self.orig_shape[0] * 0.03)
            annotator.text([x, x], text, txt_color=(255, 255, 255))  # TODO: allow setting colors

        # Plot Pose results输出姿态估计结果
...
class Boxes(BaseTensor):
    ...
    def __init__(self, boxes, orig_shape) -> None:
        ...
        if boxes.ndim == 1:
            boxes = boxes[None, :]
        n = boxes.shape[-1]
        assert n in {6, 7}, f"expected 6 or 7 values but got {n}"  # xyxy, track_id, conf, cls 7是跟踪，6是其他
