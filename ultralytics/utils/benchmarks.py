    ...
    for i, (name, format, suffix, cpu, gpu) in enumerate(zip(*export_formats().values())):
        ...
        try:
            ...
            if format == "-":
                ...
                filename = model.export(imgsz=imgsz, format=format, half=half, int8=int8, device=device, verbose=False)#导出
                exported_model = YOLO(filename, task=model.task)#YOLO读取导出的文件
            ...
            exported_model.predict(ASSETS / "bus.jpg", imgsz=imgsz, device=device, half=half)#执行推理
            ...
            metric, speed = results.results_dict[key], results.speed["inference"]#分析验证尺度和速度
...
class ProfileModels:
    ...
    ProfileModels类用于在ONNX和TensorRT上对不同模型进行性能分析。
    ...
    def profile(self):
        ...
        #对YOLO模型在包括ONNX和TensorRT在内的各种格式下进行速度和准确性分析。
        ...
        for file in files:
            ...
            if file.suffix in {".pt", ".yaml", ".yml"}:#从pt或yaml加载的模型
                model = YOLO(str(file))
                ...
                onnx_file = model.export(#执行onnx导出
                    format="onnx",
                    imgsz=self.imgsz,
                    device=self.device,
                    verbose=False,
            ...
            t_engine = self.profile_tensorrt_model(str(engine_file))#trt或onnx的性能分析
    ...
    def iterative_sigma_clipping(data, sigma=2, max_iters=3):
        ...
        #对数据应用迭代的sigma剪裁，以根据指定的sigma和迭代次数去除异常值。
        data = np.array(data)
    ...
    def profile_tensorrt_model(self, engine_file: str, eps: float = 1e-3):
        ...
        #使用TensorRT对YOLO模型性能进行分析，测量平均运行时间和标准差。
    ...
    def profile_onnx_model(self, onnx_file: str, eps: float = 1e-3):
        ...
        #对ONNX模型进行性能分析，测量多次运行的平均推理时间和标准差。
    ...
    def generate_table_row(self, model_name, t_onnx, t_engine, model_info):
        ...
        #生成一行字符串，以表格形式包含模型性能指标，包括推理时间和模型详细信息。
    ...
    def generate_results_dict(model_name, t_onnx, t_engine, model_info):
        ...
        #生成一个包含性能分析结果的字典，包括模型名称、参数、GFLOPs和速度指标。
        layers, params, gradients, flops = model_info
        return {
