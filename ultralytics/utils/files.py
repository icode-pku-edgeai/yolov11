...
def spaces_in_path(path):
    ...
    为了处理包含空格的路径名，我们可以创建一个上下文管理器，该管理器会在执行上下文代码块之前将路径中的空格替换为下划线，
    并将文件或目录复制到新路径。执行完上下文代码块后，它再将文件或目录复制回原始位置
    Args:
...
def get_latest_run(search_dir="."):#返回指定目录中最新‘last.pt’文件的路径，以便恢复训练
...
def update_models(model_names=("yolov8n.pt",), source_dir=Path("."), update_names=False):
    ...
    更新并重新保存在updated_models子目录中的指定YOLO模型
    Args:
