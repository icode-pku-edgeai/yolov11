...
class TQDM(tqdm_original):
    ...
    “一个自定义的 TQDM 进度条类，扩展了原始 TQDM 的功能”
...
def plt_settings(rcparams=None, backend="Agg"):
    ...
    def decorator(func):
        ...
        #“一个装饰器，用于将临时rc参数和后端应用于一个函数”
        def wrapper(*args, **kwargs):
            """Sets rc parameters and backend, calls the original function, and restores the settings."""
            #“设置 rc 参数和后端，调用原始函数，然后恢复设置。”
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    #“创建并配置 StreamHandler，设置合适的格式化器和日志级别。”
    stream_handler = logging.StreamHandler(sys.stdout)
    stream_handler.setFormatter(formatter)
    stream_handler.setLevel(level)

    # Set up the logger “设置日志记录器。”
...
class ThreadingLocked:
    ...
    “一个装饰器类，用于确保函数或方法的线程安全执行。这个类可以作为装饰器使用，以确保如果被装饰的函数从多个线程调用，只有一个线
    程能够同时执行该函数。”
    Attributes:
...
def is_jupyter():
    ...
    “检查当前脚本是否在 Jupyter Notebook 中运行。已在 Colab、JupyterLab、Kaggle 和 Paperspace 上验证。”
    Returns:
...
def is_raspberrypi() -> bool:
    ...
    “通过检查设备型号信息，确定 Python 环境是否在 Raspberry Pi 上运行。”
    Returns:
...
def is_jetson() -> bool:
    ...
    information.
    “通过检查设备型号信息，确定 Python 环境是否在 Jetson Nano 或 Jetson Orin 设备上运行。”
    Returns:
...
def get_git_dir():
    ...
    “确定当前文件是否属于 git 仓库，如果是，则返回仓库根目录；如果不是，则返回 None。”
    Returns:
...
def is_git_dir():
    ...
    “确定当前文件是否属于 git 仓库。如果不是，则返回 None。”
    Returns:
...
def get_git_branch():
    ...
    “返回当前 git 分支名称。如果不在 git 仓库中，则返回 None。”
    Returns:
...
def set_sentry():
    ...
    “初始化 Sentry SDK 以进行错误跟踪和报告。仅在安装了 sentry_sdk 包且设置中 sync=True 时使用。
    ...
    def before_send(event, hint):
        ...
        “根据特定异常类型和消息在发送事件到 Sentry 之前进行修改。”
        Args:
