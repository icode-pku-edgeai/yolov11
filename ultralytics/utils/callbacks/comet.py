...
try:
    ...
    COMET_SUPPORTED_TASKS = ["detect"]#仅目标检测支持comet

    # Names of plots created by Ultralytics that are logged to Comet
    EVALUATION_PLOT_NAMES = "F1_curve", "P_curve", "R_curve", "PR_curve", "confusion_matrix"#验证内容包括F1、P、R、PR、混淆矩阵
...
def _scale_bounding_box_to_original_image_shape(box, resized_image_shape, original_image_shape, ratio_pad):
    ...
    #"YOLO在训练过程中会调整图像大小，并根据调整后的形状对标签值进行归一化。"
...
def _log_confusion_matrix(experiment, trainer, curr_step, curr_epoch):
    """Log the confusion matrix to Comet experiment."""#"将混淆矩阵记录到Comet实验中。"
