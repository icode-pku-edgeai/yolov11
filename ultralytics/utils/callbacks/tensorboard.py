...
def _log_scalars(scalars, step=0):
    """Logs scalar values to TensorBoard."""#"将标量值记录到TensorBoard。"
    if WRITER:
...
def _log_tensorboard_graph(trainer):
    """Log model graph to TensorBoard."""#"将模型图记录到TensorBoard。"
    # Input image
...
def on_pretrain_routine_start(trainer):
    """Initialize TensorBoard logging with SummaryWriter."""#"使用SummaryWriter初始化TensorBoard日志记录。"
    if SummaryWriter:
        try:
            global WRITER
...
def on_train_start(trainer):
    """Log TensorBoard graph."""#"记录TensorBoard图。"
    if WRITER:
