...
class Colors:
    ...
    def __call__(self, i, bgr=False):
        """Converts hex color codes to RGB values."""#“将十六进制颜色代码转换为 RGB 值。”
...
class Annotator:
    ...
    “Ultralytics 标注工具，用于训练/验证马赛克、JPG 图像及预测注释。”
    Attributes:
    ...
    def text_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), margin=5):
        ...
        “在给定边界框内绘制一个带背景矩形的标签，矩形中心位于边界框中心。”
        Args:
            box (tuple): The bounding box coordinates (x1, y1, x2, y2).
            label (str): The text label to be displayed.
            color (tuple, optional): The background color of the rectangle (B, G, R).
    ...
    def text(self, xy, text, txt_color=(255, 255, 255), anchor="top", box_style=False):
        """Adds text to an image using PIL or cv2."""#图像上添加文本
    ...
    def show(self, title=None):
        """Show the annotated image."""#显示带标签的图像
    ...
    def queue_counts_display(self, label, points=None, region_color=(255, 255, 255), txt_color=(0, 0, 0)):
        ...
        “在图像上显示队列计数，中心位于指定点，支持自定义字体大小和颜色。”
        Args:
...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    “将图像裁剪保存为 {file}，裁剪大小为 {gain} 的倍数，边距为 {pad} 像素。保存并/或返回裁剪结果。”
...
def plot_results(file="path/to/results.csv", dir="", segment=False, pose=False, classify=False, on_plot=None):
    ...
    “从结果 CSV 文件绘制训练结果。该函数支持多种数据类型，包括分割、姿态估计和分类。图表将保存为 'results.png'，位于 CSV 文
    件所在的目录。”
    Args:
...
def plot_tune_results(csv_file="tune_results.csv"):
    ...
    “绘制存储在 'tune_results.csv' 文件中的演变结果。该函数为 CSV 中的每个键生成散点图，颜色基于适应度得分。最佳配置在图中突出显示。”
    Args:
...
def output_to_rotated_target(output, max_det=300):
    """Convert model output to target format [batch_id, class_id, x, y, w, h, conf] for plotting."""
    #“将旋转目标模型输出转换为目标格式 [batch_id, class_id, x, y, w, h, conf] 以便绘图。”
    targets = []
    for i, o in enumerate(output):
