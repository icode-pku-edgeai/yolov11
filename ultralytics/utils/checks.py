...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    ...
    验证图像尺寸是否在每个维度上是给定步幅的倍数。如果图像尺寸不是步幅的倍数，则将其更新为大于或等于给定下限值的最近步幅倍数。
    Args:
...
def check_torchvision():
    ...
    检查已安装的PyTorch和Torchvision版本，以确保它们兼容。
...
def check_suffix(file="yolov8n.pt", suffix=".pt", msg=""):
    """Check file(s) for acceptable suffix."""#检查文件是否具有可接受的后缀。
    if file and suffix:
...
def check_model_file_from_stem(model="yolov8n"):
    """Return a model filename from a valid model stem."""#从有效的模型基名返回模型文件名。
