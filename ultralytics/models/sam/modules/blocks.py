...
class CXBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return x # x+(x-dwconv-ln-transpose-fc-act-fc-mul-dropout)
...
class SAM2TwoWayTransformer(TwoWayTransformer):
    ...
    “一个双向 Transformer 模块，用于同时关注图像和查询点。”
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList()#堆叠双向注意力机制
...
class RoPEAttention(Attention):
    ...
    “为 Transformer 架构中的注意力机制实现旋转位置编码。”
...
class MultiScaleBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return x #x-LN-window_partition-attention-window_unpartition-dropout-shortcut-LN-mlp-drop-shortcut
...
class PositionEmbeddingSine(nn.Module):
    ...
    def _encode_xy(self, x, y):
        ...
        #“使用正弦/余弦函数对二维位置进行编码，以便用于 Transformer 的位置嵌入。”
...
class PositionEmbeddingRandom(nn.Module):
    ...
    def forward_with_coords(self, coords_input: torch.Tensor, image_size: Tuple[int, int]) -> torch.Tensor:
        ...
        #“对输入坐标进行位置编码，基于给定图像大小将其标准化到 [0,1]。”
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = shortcut + x
        return x + self.mlp(self.norm2(x))#x-ln-window_partition-REAttention-window_unpartition-shortcut-ln-mlp-shortcut


class REAttention(nn.Module):
    """
    Rotary Embedding Attention module for efficient self-attention in transformer architectures.
    “旋转嵌入注意力模块，用于在 Transformer 架构中实现高效的自注意力。”
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.proj(x)#x-fc-kqv-v@softmax(k@q*scale)-fc


class PatchEmbed(nn.Module):
    """
    Image to Patch Embedding module for vision transformer architectures.
    “用于视觉 Transformer 架构的图像到补丁嵌入模块。”
