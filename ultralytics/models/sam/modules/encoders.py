...
class ImageEncoderViT(nn.Module):
    ...
    “使用视觉变换器ViT架构的图像编码器,将图像编码为紧凑的潜在空间。”
...
class PromptEncoder(nn.Module):
    ...
    “对不同类型的提示进行编码,以输入到SAM的掩码解码器,生成稀疏和密集嵌入。”
    Attributes:
...
class ImageEncoder(nn.Module):
    ...
    “使用trunk-neck架构对图像进行编码,生成多尺度特征和位置编码。”
...
class FpnNeck(nn.Module):
    ...
    “一种特征金字塔网络fpn颈部变体,用于物体检测模型中的多尺度特征融合。”
