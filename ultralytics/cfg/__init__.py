...
CFG_FLOAT_KEYS = {  # integer or float arguments, i.e. x=2 and x=2.0 整型或浮点型赋值参数
...
CFG_FRACTION_KEYS = {  # fractional float arguments with 0.0<=values<=1.0 比例赋值参数
    "dropout",#随机失活
    "lr0",#起始和结尾学习率
    "lrf",
    "momentum",#梯度下降率
    "weight_decay",#权值衰减率
    "warmup_momentum",#warmup的梯度下降率
    "warmup_bias_lr",#warmup的基准学习率
    "label_smoothing",#标签平滑率
    "hsv_h",#色相h、饱和度s、明度v
    "hsv_s",
    "hsv_v",
    "translate",#平移
...
CFG_INT_KEYS = {  # integer-only arguments
    ...
    "vid_stride",#视频采样间隔
    "line_width",#线宽
    "nbs",#nominal batch size 名义批量大小
    "save_period",#保存频率
}
CFG_BOOL_KEYS = {  # boolean-only arguments bool参数
    "save",#保存
    "exist_ok",#输出文件夹是否覆盖
    "verbose",#日志输出
    "deterministic",#确定性输出,保证相同输入条件下输出相同
    "single_cls",#单一类别
    "rect",#矩形训练
    "cos_lr",#余弦学习率衰减
    "overlap_mask",#重叠目标掩码
    ...
    "visualize",#可视化
    "augment",#数据增强
    "agnostic_nms",#类无关的nms
    "retina_masks",#高分辨率掩膜
    "show_boxes",#显示框
    "keras",#
    "optimize",#onnx-optimize
    "int8",#转int8整型
    "dynamic",#动态输出
...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None):
    ...
    # Special handling for numeric project/name 项目/名称的特殊处理
...
def handle_explorer(args: List[str]):
    ...
    启动GUI,通过Ultralytics Explorer API提供与数据集的交互和分析工具
    Args:
...
def handle_streamlit_inference():
    ...
    打开Ultralytics的实时推理Streamlit应用,用于实时目标检测
...
def smart_value(v):
    ...
    将none/ture/false等字符串转义为对应的python类型
...
def entrypoint(debug=""):
    ...
    if not args:  # no arguments passed 处理没有有效输入的情况
    ...
    # Define common misuses of special commands, i.e. -h, -help, --help 处理特殊指令
    special.update({k[0]: v for k, v in special.items()})  # singular
    special.update({k[:-1]: v for k, v in special.items() if len(k) > 1 and k.endswith("s")})  # singular
    special = {**special, **{f"-{k}": v for k, v in special.items()}, **{f"--{k}": v for k, v in special.items()}}

    overrides = {}  # basic overrides, i.e. imgsz=320
    for a in merge_equals_args(args):  # merge spaces around '=' sign 分词处理命令行输入
        if a.startswith("--"):
...
def copy_default_cfg():
    ...
    复制默认配置文件,并创建一个新文件,其名称在原名称后附加了'_copy'
